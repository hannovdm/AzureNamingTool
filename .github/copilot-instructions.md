# AzureNamingTool Development Guidelines

Last updated: 2026-04-28

AzureNamingTool is a Azure Naming Tool — generate standardized Azure resource names via Blazor Server UI and v1+v2 REST APIs. Dual storage (FileSystem JSON / SQLite EF Core)..

## Technology Stack

| Layer           | Technology                | Notes                                 |
|-----------------|---------------------------|---------------------------------------|
| Language        | C# .NET 10.0 | (nullable reference types enabled; implicit usings enabled; suppresses CS8602/CS8600/CS1998)          |
| Build           | dotnet            | `dotnet build src/AzureNamingTool.csproj`                   |
| Test            | dotnet test (xunit)           | `dotnet test tests/AzureNamingTool.UnitTests/AzureNamingTool.UnitTests.csproj`                    |
| Lint            | roslyn analyzers (no external linter)                | `dotnet build src/AzureNamingTool.csproj /warnaserror-:CS8602,CS8600,CS1998`                    |
| Format          | dotnet format             | `dotnet format src/AzureNamingTool.sln --verify-no-changes`                  |
| CI              | GitHub Actions           | GitHub Actions workflows under .github/workflows/. CI runs build + unit tests on push/PR.                          |
...
```bash
dotnet build src/AzureNamingTool.csproj              # Build
dotnet test tests/AzureNamingTool.UnitTests/AzureNamingTool.UnitTests.csproj               # Run all tests
dotnet build src/AzureNamingTool.csproj /warnaserror-:CS8602,CS8600,CS1998               # Lint
dotnet format src/AzureNamingTool.sln --verify-no-changes             # Format check
...
Typed exceptions; controller-boundary translation; v2 returns `ApiResponse<T>`; v1 returns legacy `ServiceResponse`.

### Naming

PascalCase for types, members, public properties; camelCase for locals + private fields; `_` prefix for private fields when needed; interfaces prefixed with `I`

### Documentation

XML doc comments on public surfaces; ADRs in `docs/v5.0.0/adrs/` for architecture-level decisions.

### Testing

* TDD required: write tests first, verify they fail, then implement
* Test tiers in `tests/` directory:
Unit (xunit + Moq + FluentAssertions) and UI (Playwright + LightBDD + NUnit, net8.0; requires running app).
