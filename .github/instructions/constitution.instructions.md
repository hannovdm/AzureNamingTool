---
description: "Constitutional principles governing all agent operations in this workspace — adapted for C#"
applyTo: '**'
---
...
All production code MUST be written in C# (.NET 10.0).
No `unsafe` blocks; no reflection-based bypasses of nullability checks; respect `<Nullable>enable</Nullable>`. Build with no NEW analyzer warnings. The repo currently suppresses CS8602, CS8600, and CS1998 — do not widen the suppression list without ADR justification.. All exceptions MUST be typed, logged via `LogHelper`, and translated to proper HTTP responses at controller boundaries. v2 endpoints return `ApiResponse<T>`..
...
directory structure (Unit tests in `tests/AzureNamingTool.UnitTests/` (xunit); UI tests in `tests/AzureNamingTool.UiTests/` (Playwright + LightBDD + NUnit, net8.0).) MUST be maintained. All tests MUST
pass via `dotnet test tests/AzureNamingTool.UnitTests/AzureNamingTool.UnitTests.csproj` before any code is merged. Steps: write test,
...
|-----------------|------------------------------------------------------------------|
| Language        | C# .NET 10.0                        |
| Build           | `dotnet build src/AzureNamingTool.csproj`                                              |
| Test            | `dotnet test tests/AzureNamingTool.UnitTests/AzureNamingTool.UnitTests.csproj`                                               |
| Lint            | `dotnet build src/AzureNamingTool.csproj /warnaserror-:CS8602,CS8600,CS1998`                                               |
| Format          | `dotnet format src/AzureNamingTool.sln --verify-no-changes`                                             |
| CI              | GitHub Actions                                                  |
| Error Handling  | throw typed exceptions; catch and translate at controller boundaries; v2 API returns ApiResponse<T>                                                |
| Documentation   | /// <summary> XML doc comments                                            |
...
dotnet test tests/AzureNamingTool.UiTests/AzureNamingTool.UiTests.csproj
```
...
| **Tier 2 (Standard)**  | Medium model | ship, csharp-engineer, harness-architect | Routine code, scaffolding, coordination |
...
|---|---|---|
| I. Safety-First Language | MUST | CI quality gates (`dotnet build src/AzureNamingTool.csproj /warnaserror-:CS8602,CS8600,CS1998`, `dotnet build src/AzureNamingTool.csproj`) |
...
**Version**: 1.0.0 | **Ratified**: 2026-04-28 | **Generated by**: autoharness
