...
3. **Verify locally**: Run the relevant quality gate
   (`dotnet format src/AzureNamingTool.sln --verify-no-changes`, `dotnet build src/AzureNamingTool.csproj /warnaserror-:CS8602,CS8600,CS1998`, `dotnet test tests/AzureNamingTool.UnitTests/AzureNamingTool.UnitTests.csproj`)
...
' -f owner="hannovdm" -f repo="AzureNamingTool" -F pr=<pr_number>
```
...
```bash
gh api repos/hannovdm/AzureNamingTool/commits/<head_sha>/check-runs \
...
```bash
gh api repos/hannovdm/AzureNamingTool/check-runs/<check_run_id>/annotations \
