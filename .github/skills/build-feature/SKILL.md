...
* `harness_cmd`: (Required) The test command to run (e.g., `dotnet test tests/AzureNamingTool.UnitTests/AzureNamingTool.UnitTests.csproj`).
...
and `csharp.instructions.md`. These apply to all fix attempts.
...
Attempt 1..5:
  ...
  6. Verify compilation: dotnet build src/AzureNamingTool.csproj --no-restore
...
* Technology-specific instructions (`csharp.instructions.md`)
...
Run `dotnet build src/AzureNamingTool.csproj --no-restore` to confirm the fix compiles. If compilation fails, fix compilation errors before returning to the harness loop.
...
2. **Format**: `dotnet format src/AzureNamingTool.sln --verify-no-changes`
   * If violations found: `dotnet format src/AzureNamingTool.sln` and re-check
