...
**Postcondition** (harness-architect): The task has `harness-ready` label, the harness compiles (`dotnet build src/AzureNamingTool.csproj --no-restore`), and all tests fail with expected failure markers.
...
**Precondition**: `dotnet build src/AzureNamingTool.csproj --no-restore` exits 0 AND `dotnet test tests/AzureNamingTool.UnitTests/AzureNamingTool.UnitTests.csproj` exits non-zero with expected failure markers in the output for every test function.
...
**Applies when**: `false` is true and `manual` is `backlogit`.

**Scope**: Ensures archive files are not silently lost after `` runs during Ship Step 6 post-merge closure in backlogit workspaces.

**Precondition**: `` has been invoked with a merge commit SHA, archiving the shipment's queue items.

**Gate Point**: Ship Step 6, immediately after `` completes and before the backlog state is committed.

**Required Check**: Run `git status -- ".backlog/archive/"` and inspect for working-tree deletions. A known backlogit archival quirk may move files to the archive directory and then delete the archive copies from the working tree.

**Postcondition**: All archive files for the shipped items exist in `.backlog/archive/` with no working-tree deletions reported by `git status`.

**Violation Action**:

1. Run `git restore .backlog/archive/` to recover deleted archive files
2. Re-verify with `git status -- ".backlog/archive/"` — confirm no deletions remain
3. Stage the restored files: `git add .backlog/archive/`
...
|---------|--------------|------------------|----------------------------|
| 1.0.0   | 2026-04-28     | Initial registry | Generated by autoharness   |
| 1.1.0   | 2026-04-28     | Added P-006      | Plan hardening gate enforcement |
| 1.2.0   | 2026-04-28     | Added P-007      | Backlogit archive integrity after shipment |
| 1.3.0   | 2026-04-28     | Added P-008      | Markdown conformance enforcement |
| 1.4.0   | 2026-04-28     | Added P-009      | Merge-commit-only policy |
