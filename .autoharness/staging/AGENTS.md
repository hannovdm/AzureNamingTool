...
Azure Naming Tool — ASP.NET Core Blazor Server (.NET 10.0). Layered architecture: Controllers (v1+v2) -> Services -> Repositories -> JSON or SQLite (EF Core, dual storage).
...
No unsafe blocks; respect <Nullable>enable</Nullable>. Build with no NEW analyzer warnings. Repo currently suppresses CS8602/CS8600/CS1998 -- do not widen suppression list without ADR..
All exceptions MUST be typed, logged via LogHelper, and translated at controller boundaries. v2 endpoints return ApiResponse<T>..
