...
1. **Check for an existing shipment in `queued` status** that already covers the
   harvested feature scope using ``. If one already exists for this
   feature, add the newly harvested tasks to it rather than creating a duplicate.

2. **Create the shipment** (when none exists):
   * Use `` with a title derived from the covering feature title
...
3. **Scope guard (mandatory first step)**: Record the exact list of IDs returned by the
   ...
   for shipment assembly. `` MUST ONLY be called for items that
...
4. **Add remaining items in parent-first, dependency order** using ``:
...
5. **Verify the manifest** by reading back the shipment using `` and
...
**SESSION_START**
1. Call `` with `consumer_id: "stage"`, `status: "active"`, and `max_age_hours: 168`.
...
4. If the operator chooses fresh, resolve stale checkpoints with `` and continue to stash processing.
...
When the `backlogit` capability pack is installed and the registry advertises hook polling operations, poll for unacknowledged signals before stash triage using `` with `consumer_id: "stage"`.

Treat concrete `events` as higher-priority signals than the raw stash queue. After processing them, acknowledge only the highest `seq` from the concrete `events` array with ``. Never acknowledge `derived_signals`, and skip the ack call entirely when no concrete events are returned.
...
When the `backlogit` capability pack is installed and `` is available, also persist a phase-tagged structured checkpoint through backlogit. Include the current phase, relevant stash or feature IDs, created artifact IDs, next step, and a `resume_hint` specific enough for a later recovery decision.
...
2. When the `backlogit` capability pack is installed and the registry advertises checkpoint recovery operations, resolve any still-active checkpoints from the current session with ``. When the next action must survive a context-window shutdown, leave at most one final best-effort checkpoint written via `` with a clear `resume_hint`.
