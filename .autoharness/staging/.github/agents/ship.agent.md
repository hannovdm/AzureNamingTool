...
maturity: stable
tools: vscode, execute, read, agent, edit, search, todo, memory, none (manual file-based backlog)
...
1. Load the shipment using ``. Confirm it is in `queued` or `active` status.
...
4. If the shipment is still in `queued` status, claim it using `` before
...
6. **Intake reconciliation check**: Invoke `shipment-reconcile` with `mode: pre` and
   ...
   This verifies every manifest item is present in `.backlog/queue/` with the
...
1. List existing shipments in `queued` status using `` to
...
3. If the operator confirms direct assembly, create the shipment:
   ...
   b. Create the shipment using `` with a title from the feature
...
2. **Verify compilation**: Run `dotnet build src/AzureNamingTool.csproj --no-restore` to confirm the project builds
...
3. If any tasks need harnesses, invoke the **harness-architect** skill for the batch.
   * Require compilable but failing harnesses, structural stubs, and successful `dotnet build src/AzureNamingTool.csproj --no-restore` verification after scaffolding.
...
* `harness_cmd`: The test command from the task's harness-ready metadata (e.g., `dotnet test tests/AzureNamingTool.UnitTests/AzureNamingTool.UnitTests.csproj --test {feature}_test`)
...
1. **Lint**: `dotnet build src/AzureNamingTool.csproj /warnaserror-:CS8602,CS8600,CS1998`
2. **Format**: `dotnet format src/AzureNamingTool.sln --verify-no-changes`
3. **Full Test Suite**: `dotnet test tests/AzureNamingTool.UnitTests/AzureNamingTool.UnitTests.csproj`
...
7. **Stash follow-up items**: If the closure artifact or runtime-verification report identified follow-up tasks, stash every follow-up so it is visible to the Stage agent:
   * When `backlogit` is the installed backlog tool, create a stash entry per follow-up using `` with `type: "stash"`, `title` from the follow-up summary, `description` linking to the closure artifact, and `status: "queued"`. After creation, re-read each entry to confirm it persisted correctly.
   * When `backlog-md` is the installed backlog tool, create a follow-up item using `` with `title` from the follow-up summary, `description` linking to the closure artifact, `status: "queued"`, and `labels: ["stash", "follow-up"]`.
   * When no backlog tool is installed, append each follow-up to `.backlog/queue/.stash.md` using the format: `- [{YYYY-MM-DD}] **Follow-up**: {summary} — Source: {closure_artifact_path}`.
...
1. **Confirm the feature branch merge is complete**: Verify the merge commit exists on
   `main` (e.g., `git log main --oneline -1` shows the merge).
...
1. **Close the shipment** (when `false` is true):
   a. **Pre-archive reconciliation gate (mandatory)**: Invoke the `shipment-reconcile`
      ...
      This acquires the single-writer lock on `.backlog/queue/{shipment_id}.md`
   ...
   b. Call `` with the merge commit SHA. This archives all queue
      items (feature + tasks) to `.backlog/archive/`.
   c. **Verify archive integrity (P-007)**: Run `git status -- ".backlog/archive/"`.
      If any archive files appear as working-tree deletions, restore them immediately:
      `git restore .backlog/archive/`. See P-007 in workflow-policies for the
   ...
   d. **Post-archive reconciliation**: Invoke `shipment-reconcile` with `mode: post` and
      ...
      `git restore .backlog/archive/` before step 1.e.
...
**SESSION_START**
1. Call `` with `consumer_id: "ship"`, `status: "active"`, and `max_age_hours: 168`.
...
3. If the operator chooses resume, load the selected checkpoint with ``.
4. If the operator chooses fresh, resolve stale checkpoints with `` and continue to shipment validation.

**RESUME_FROM_CHECKPOINT**
1. If `` returns an error or invalid payload, warn and fall back to a fresh start.
...
3. Resolve all other still-active checkpoints from prior sessions with ``.
4. Resume from the recorded phase instead of restarting execution from scratch.

**FRESH_START**
1. Resolve any active checkpoints left over from prior sessions with ``.
2. Continue with normal shipment validation.

### Hook event consumption

When the `backlogit` capability pack is installed and the registry advertises hook polling operations, poll for unacknowledged signals before shipment validation using `` with `consumer_id: "ship"`.

Treat concrete `events` as higher-priority signals than the raw work queue. After processing them, acknowledge only the highest `seq` from the concrete `events` array with ``. Never acknowledge `derived_signals`, and skip the ack call entirely when no concrete events are returned.
...
When the `backlogit` capability pack is installed and `` is available, also persist a phase-tagged structured checkpoint through backlogit. Include shipment or feature IDs, completed and blocked item IDs, branch state, next step, and a `resume_hint` specific enough for a later recovery decision.
...
2. When the `backlogit` capability pack is installed and the registry advertises checkpoint recovery operations, resolve any still-active checkpoints from the current session with ``. When merge approval or closure work must survive a context-window shutdown, leave at most one final best-effort checkpoint written via `` with a clear `resume_hint`.
