...
knowledge) with an optional work item in `.backlog/queue/` for
workflow tracking.
...
* Search `.backlog/queue/` for related active work items
...
* If promoting to queue, create a spike item using `` with
...
* If promoting to queue, append a spike entry to
  `.backlog/queue/.stash.md` with the format:
  ```
