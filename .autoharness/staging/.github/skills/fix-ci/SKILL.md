...
* Local tools required by this skill must be available in PATH, including the configured quality gates `dotnet build src/AzureNamingTool.csproj`–`dotnet test tests/AzureNamingTool.UiTests/AzureNamingTool.UiTests.csproj`; if formatting fixes are applied during remediation, `dotnet format src/AzureNamingTool.sln` must also be available
...
| `max_wait` | int | `900` | Maximum total seconds to wait for CI to reach a terminal state. |
...
1. Format check (`dotnet format src/AzureNamingTool.sln --verify-no-changes`)
2. Lint (`dotnet build src/AzureNamingTool.csproj /warnaserror-:CS8602,CS8600,CS1998`)
3. Test (`dotnet test tests/AzureNamingTool.UnitTests/AzureNamingTool.UnitTests.csproj`)
...
1. `dotnet format src/AzureNamingTool.sln --verify-no-changes` → if fails, run `dotnet format src/AzureNamingTool.sln`
2. `dotnet build src/AzureNamingTool.csproj /warnaserror-:CS8602,CS8600,CS1998` → fix violations
3. `dotnet test tests/AzureNamingTool.UnitTests/AzureNamingTool.UnitTests.csproj` → fix failing tests
...
`.backlog/queue/.stash.md` using the format:
...
|---|---|---|
| Run auto-fix command | Formatting diff exists and auto-fix is configured (`dotnet format src/AzureNamingTool.sln`) | Auto-fix introduces semantic changes or breaks tests |
