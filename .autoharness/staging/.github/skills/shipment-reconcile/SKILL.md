...
* **Ship Step 6** (mandatory): pre-mode immediately before ``;
  post-mode immediately after the `git restore .backlog/archive/` step.
...
`.backlog/reconcile/{shipment_id}-{mode}-{timestamp}.md`.
...
* **Single-writer lock.** When invoked from Ship Step 6, this skill holds the
  `.backlog/queue/{shipment_id}.md` file lock (via the `file-lock` skill) for
...
1. **Acquire single-writer lock** (Ship Step 6 invocations only, not intake):
   Invoke the `file-lock` skill to acquire `.backlog/queue/{shipment_id}.md`.
...
4. **Orphan scan**:
   Scan `.backlog/queue/` for any files whose YAML frontmatter declares
...
1. **Verify archive presence**:
   List `.backlog/archive/` and confirm a file exists for the shipment itself
   (`{shipment_id}.*`).
...
3. **Deleted-file guard** (known `` quirk — see P-007):
   Run `git status -- ".backlog/archive/"` and inspect for deletions.
   If any archive files are reported as deleted, recommend
   `git restore .backlog/archive/` before the commit step.
...
6. **Release lock** (acquired in step 1 of pre-mode):
   Invoke `file-lock` release for `.backlog/queue/{shipment_id}.md`.
