...
This workspace uses **manual** for structured backlog management. All agents MUST use the backlog tool for task tracking rather than creating ad-hoc markdown files or static task lists.
...
_(no extended operations — manual file-based backlog)_
...
1. **Always use the backlog tool** for task management. Do not create markdown task files outside the `.backlog/` directory.
