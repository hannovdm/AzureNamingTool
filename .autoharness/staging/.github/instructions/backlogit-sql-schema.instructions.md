...
The SQLite index at `.backlog/backlogit.db` is an ephemeral, read-only query cache. The Markdown files under `.backlog/` are the source of truth. Use `backlogit_query_sql` to run `SELECT` statements against this cache. Rebuild the cache at any time with `backlogit_sync_index`.
