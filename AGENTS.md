...
### I. Safety-First C# (NON-NEGOTIABLE)

All production code MUST be written in C# (.NET 10.0).
No `unsafe` blocks; no reflection-based bypasses of nullability checks; respect `<Nullable>enable</Nullable>`. Build with no NEW analyzer warnings. The repo currently suppresses CS8602, CS8600, and CS1998 — do not widen the suppression list without ADR justification..
All exceptions MUST be typed, logged via `LogHelper`, and translated to proper HTTP responses at controller boundaries. v2 endpoints return `ApiResponse<T>`..
...
pass via `dotnet test tests/AzureNamingTool.UnitTests/AzureNamingTool.UnitTests.csproj` before any code is merged.
...
work items in the backlog queue (`.backlog/queue/`). Durable knowledge
